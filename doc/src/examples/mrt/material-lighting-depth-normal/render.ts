...
import { UseMRT } from "sketchgl/renderer"
...
import bunny from "./Bunny-LowPoly.json" assert { type: "json" }
...
const sketch: SketchFn = ({ gl, canvas }) => {
  ...
  const renderer = new UseMRT(gl, canvas, vertCalc, fragCalc, { texCount: 4 })
  ...
  uniforms.shape.init(renderer.glProgramForMRT)
  ...
  return {
    ...
    drawOnFrame: () => {
      ...
      uniforms.shape.fvector3("uMaterialColor", [0.792, 0.824, 0.969])
      ...
      for (let i = 0; i < 4; ++i) {
        ...
        renderer.useTexture(program, { idx: i, name: "uTexture" })
        plane.bind()
